...
class SessionManager @Inject constructor(
    ...
    fun getLocation(): Pair<Double, Double>? {
        val lat = prefs?.getFloat("map_lat", Float.NaN)
        val lng = prefs?.getFloat("map_lng", Float.NaN)
    ...
    fun isSessionValid(): Boolean {
        val loginTime = prefs?.getLong("login_timestamp", 0L) ?: 0L
        if (loginTime == 0L) return false
        val thirtyDaysInMillis = 30L * 24 * 60 * 60 * 1000
        return (System.currentTimeMillis() - loginTime) < thirtyDaysInMillis
    }
