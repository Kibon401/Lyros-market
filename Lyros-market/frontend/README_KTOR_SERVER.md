# Farm-to-Client Ktor Backend Server Specification

## 1. Project Architecture (Clean Architecture)
The backend is structured to be scalable and maintainable, following strict separation of concerns.
...
data
├── datasource   (External API / File access)
├── repository   (Implementation of domain repos)
├── database     (Connection logic - HikariCP)
├── dao          (Data Access Objects)
└── entities     (Exposed Table definitions)

core
├── auth         (JWT & Refresh Token logic)
├── security     (BCrypt, Rate Limiting)
├── plugins      (Ktor Feature Config)
...
- **Security:** JWT (Access + Refresh Tokens), BCrypt Hashing, Rate Limiting
- **Serialization:** Kotlinx Serialization
- **Logging:** Logback & Sentry (Production Error Tracking)
- **Payments:** Daraja API (Ktor Client integration)

---

## 3. Database Schema (Exposed Mapping)
All operations are wrapped in `transaction` blocks for atomicity.

```kotlin
// Tables: site_user, product_category, product, product_attribute, 
// product_attribute_value, shopping_cart, shopping_cart_item, 
// shop_order, order_line, mpesa_payment, user_review
```
...
| Module | Base Path | Core Endpoints |
| :--- | :--- | :--- |
| **Auth** | `/auth` | `/login`, `/register`, `/refresh-token`, `/forgot-password`, `/reset-password` |
| **Users** | `/users` | `/profile` (GET/PUT), `/upload-picture`, `/change-address`, `/delete-account` |
| **Products** | `/products` | `/` (GET), `/{id}` (GET), `/search`, `/filter` |
| **Categories** | `/categories` | `/` (GET) |
| **Cart** | `/cart` | `/add`, `/remove`, `/update-qty`, `/clear`, `/total` |
| **Orders** | `/orders` | `/checkout`, `/track/{id}`, `/history`, `/cancel/{id}` |
| **Payments** | `/payments` | `/stk-push`, `/callback`, `/verify`, `/history` |
| **Drivers** | `/drivers` | `/assigned-tasks`, `/status-update`, `/history` |
| **Reviews** | `/reviews` | `/add`, `/product/{id}`, `/reply/{reviewId}` (Admin only) |
| **Admin** | `/admin` | `/users`, `/analytics`, `/inventory`, `/assign-driver` |

---

## 5. Security & Production Features
- **JWT + Refresh Token:** Ensures secure, long-lived sessions without requiring frequent logins.
- **Server-Side Fee Calculation:** Uses client coordinates to compute delivery fees securely.
- **Rate Limiting:** Protects `/auth` and `/payments` from brute force and DOS attacks.
- **Webhook Security:** Validates Safaricom Daraja callbacks via IP filtering and secure headers.
- **Notifications:** Integrated notification dispatcher for Order, Payment, and Delivery updates.
